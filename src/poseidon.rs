...
pub const R_f: usize = 3;
pub const R_P: usize = 2;
pub const R: usize = 2 * R_f + R_P;

// There are `WIDTH` number of rows per round; add one row for allocating the preimage and one row
// for checking that the calculated digest agrees with the public input digest.
pub const N_ROWS_USED: u32 = (R * WIDTH + 2) as u32;
pub const PUB_INPUT_ROW_INDEX: usize = N_ROWS_USED as usize - 1;

lazy_static! {
    pub static ref PRE_KEYS: Vec<Fp> = rand_pre_keys([1; 32]);
    pub static ref POST_KEYS: Vec<Vec<Fp>> = rand_post_keys([2; 32]);
    pub static ref MDS: Vec<Vec<Fp>> = rand_matrix([3; 32]);
    pub static ref PRE_SPARSE: Vec<Vec<Fp>> = rand_matrix([4; 32]);
    pub static ref SPARSE: Vec<Vec<Vec<Fp>>> = (0..R_P)
...
pub struct PoseidonChipConfig {
    ...
    pub sbox_out_col: Column<Advice>,
    pub mds_out_col: Column<Advice>,
    pub pre_key_col: Column<Fixed>,
    pub post_key_col: Column<Fixed>,
    pub mds_cols: Vec<Column<Fixed>>,
    pub s_sbox_pre_post: Selector,
    pub s_sbox_post: Selector,
    pub s_sbox_no_add: Selector,
    pub s_mds: Vec<Selector>,
    pub perm_output_to_input: Permutation,
    pub perm_output_to_sbox_output: Permutation,
    pub perm_preimg: Vec<Permutation>,
}
...
impl PoseidonChip {
    pub fn new(config: PoseidonChipConfig) -> Self {
        PoseidonChip { config }
    }

    pub fn configure(cs: &mut ConstraintSystem<Fp>) ->
...
pub struct HasherCircuit {
    // Private inputs.
    pub preimg: Vec<Fp>,
}

#[derive(Clone)]
pub struct HasherCircuitConfig {
    pub preimg_cols: Vec<Column<Advice>>,
    pub digest_col: Column<Advice>,
